...
export const showTotalPrice = async () => {
  const totalPriceInnerHTML = document.querySelector('.total-price');
  const productsOnCart = getSavedCartIDs();
  const total = (await productsOnCart.reduce(async (acc, product) => {
    const { price } = await fetchProduct(product);
    return await acc + price;
  }, 0));
  totalPriceInnerHTML.innerText = total;
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  cartButton.addEventListener('click', async () => {
    const teste = await fetchProduct(id);
    saveCartID(id);
    showTotalPrice();
    olCart.appendChild(createCartProductElement(teste));
  });
